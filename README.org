...
** Recursive functions
   ...
   The base case(s) and recursive case(s) must be provided or the function will not terminate.
    ...
    #+END_SRC
** Module imports
   Modules can be imported in to other modules to make their functions available.

   Syntax is =(require ...)=, where =...= is a sequence of relative filepaths to the imported module.
   #+BEGIN_SRC racket
     (require "examples/recursive.tp"
              "examples/identity.tp")

     (factorial (:identity 10))
   #+END_SRC
   
   With [[https://github.com/tpoulsen/terp/blob/master/examples/recursive.tp]["examples/recursive.tp"]] and [[https://github.com/tpoulsen/terp/blob/master/examples/identity.tp]["examples/identity.tp"]] defined as in the examples directory.

   Currently, all of a module's defined functions are imported.
** Evaluating a file:
